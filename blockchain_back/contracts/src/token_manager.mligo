...
let validate_mint_param (p : mint_param) : unit =
  ...
  | Some n ->
    (* PATCH protection from non-admin mint all range (will soft-lock contract for next mints) *)
    if n > 1000n then failwith "TOO_MUCH_TOKEN_MINT" else
    if n <> List.size p.owners then failwith "INVALID_OWNERS_LENGTH" else unit
...
let mint_tokens (p, s : mint_param * nft_token_storage) : nft_token_storage =
  ...
  then (failwith "USED_TOKEN_IDS" : nft_token_storage)
    ...
    (* TODO add height in token defs *)
    (* TODO add vote 0 in token defs *)
    let new_metadata = {
