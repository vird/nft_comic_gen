...
let nft_asset_main 
  ...
  | Tokens p ->
    (* allow not only admin mint token *)
    (* PATCH. fail_if_not_admin moved to token_manager *)

    (* let ops, assets = token_manager (p, s.assets, s.admin) in *)
    let ops = ([] : operation list) in
    let assets = match p with
      | Mint_tokens p -> mint_tokens (p, s.assets)
      | Burn_tokens p ->
        (* PATCH. allow not only admin mint token *)
        let u1 = fail_if_not_admin s.admin in
        burn_tokens (p, s.assets)
      in
